...
from twitter.TwitterConfig import s_consumer_key, s_consumer_secret_key, s_full_dev_environment, s_30_day_dev_environment
...
def v_query_30_day_api(o_scrape_helper):
    """
    This function makes the api call to twitter. Api returns pages of tweets and each 
    page is written to a pickle file for later scraping. Once completed the scraper function is 
    called.
    ...
    i_file_count = 0
    os.makedirs(o_scrape_helper.s_pickle_directory,exist_ok = True)
    for o_page in tweepy.Cursor(o_scrape_helper.o_api.search_30_day,environment_name=s_30_day_dev_environment, query=o_scrape_helper.s_query,maxResults=500).pages():
        pickle.dump(o_page, open(f'{o_scrape_helper.s_pickle_directory}pickle{i_file_count}.p', 'wb'))
        i_file_count += 1
    v_scrape_30_day_tweets(i_file_count,o_scrape_helper)
...
def v_query_full_archive_api(o_scrape_helper):
    ...
    i_file_count = 0
    os.makedirs(o_scrape_helper.s_pickle_directory,exist_ok = True)
    for o_page in tweepy.Cursor(o_scrape_helper.o_api.search_full_archive,environment_name=s_full_dev_environment, query=o_scrape_helper.s_query,maxResults=500).pages():
        pickle.dump(o_page, open(f'{o_scrape_helper.s_pickle_directory}pickle{i_file_count}.p', 'wb'))
        i_file_count += 1
...
def v_scrape_30_day_tweets(i_pickles,o_scrape_helper):
    """
    This function is where the data from the tweets will be written to our csv file
    and media from the tweet will be downloaded. Directory creation takes place here and setup
    for writing the csv file.
